...
```bash
go get github.com/jpl-au/fluent-htmx/htmx2
```
...
An htmx extension is a separate script, distributed as its own `htmx-ext-<name>` package. Enabling one takes two steps: load its script in your page `<head>` (below core htmx), then switch it on with `hx-ext`. Because `hx-ext` is inherited, set it once on the `<body>` and every element below can use the extension; you do not repeat `HxExt()` on each element.

Load the scripts for the extensions you use, after core htmx:

```html
<head>
  <script src="https://cdn.jsdelivr.net/npm/htmx.org@2"></script>
  <script src="https://cdn.jsdelivr.net/npm/htmx-ext-ws@2"></script>
  <script src="https://cdn.jsdelivr.net/npm/htmx-ext-sse@2"></script>
  <script src="https://cdn.jsdelivr.net/npm/htmx-ext-preload@2"></script>
  <script src="https://cdn.jsdelivr.net/npm/htmx-ext-response-targets@2"></script>
  <script src="https://cdn.jsdelivr.net/npm/htmx-ext-head-support@2"></script>
</head>
```

Switch them on once, on the body element. Every descendant inherits it:

```go
htmx.New(body).HxExt("ws,sse,preload,response-targets,head-support")
```

Then use the extension methods on any element below the body; no further `HxExt` is needed.

### WebSocket

```go
htmx.New(chat).WsConnect("/ws/chat")  // opens the connection
htmx.New(form).WsSend()               // a form inside it sends
```

### Server-Sent Events

```go
htmx.New(feed).SSEConnect("/events").SSESwap("message").SSEClose("done")
```

### Preload

```go
htmx.New(link).Preload("mousedown").PreloadImages(true)
```
