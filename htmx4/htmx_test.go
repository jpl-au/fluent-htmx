...
func TestClientSetters(t *testing.T) {
	cases := []setterCase{
		...
		{"HxPending", func(w *Wrapper) { w.HxPending("#pending") }, `hx-pending="#pending"`},
		{"HxTargets", func(w *Wrapper) { w.HxTargets(".card") }, `hx-targets=".card"`},
		{"HxLive", func(w *Wrapper) { w.HxLive("count + 1") }, `hx-live="count + 1"`},
		{"HxLiveText", func(w *Wrapper) { w.HxLiveText("count") }, `hx-live:text="count"`},
		{"HxLiveHTML", func(w *Wrapper) { w.HxLiveHTML("markup") }, `hx-live:html="markup"`},
		{"HxLiveClass", func(w *Wrapper) { w.HxLiveClass("{active: on}") }, `hx-live:class="{active: on}"`},
		{"HxLiveClassToggle", func(w *Wrapper) { w.HxLiveClassToggle("hidden", "empty") }, `hx-live:.hidden="empty"`},
		{"HxLiveStyle", func(w *Wrapper) { w.HxLiveStyle("{color: c}") }, `hx-live:style="{color: c}"`},
